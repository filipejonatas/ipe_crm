...
import type { CriarFornecedorDto, Fornecedor } from '@/services/fornecedores.service';
import { FornecedorFormModal } from './FornecedorFormModal';
import { FornecedorFormValues } from './fornecedor.schema';
import {
...
export function FornecedoresPage() {
  ...
  const { data, isLoading } = useFornecedores(filtros);
  ...
  function salvar(valores: FornecedorFormValues) {
    const dto = limparVazios(valores) as CriarFornecedorDto;
  ...
  function excluir(fornecedor: Fornecedor) {
    if (window.confirm(`Excluir ${fornecedor.razao_social}?`)) {
      void remover.mutate(fornecedor.id);
    }
  }
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-950">Fornecedores</h1>
          <p className="text-sm text-zinc-500">Cadastros base para compras e administrativo.</p>
        </div>
        <button
      ...
      <div className="flex flex-col gap-3 rounded-md border border-zinc-200 bg-white p-4 sm:flex-row">
        <input
          ...
          placeholder="Buscar por razao social ou fantasia"
          value={busca}
          onChange={(event) => {
            setPagina(1);
          ...
          <tbody className="divide-y divide-zinc-200">
            ...
            {data?.dados.map((fornecedor) => (
              <tr key={fornecedor.id}>
                ...
                <td className="px-4 py-3 text-sm text-zinc-600">{fornecedor.cnpj ?? '-'}</td>
